...
fullFrame = do
    nes <- getNES 
    --nes' <- liftIO $ timeIt (B.fullFrame nes)
...
handleCommand FRAME = fullFrame
handleCommand TICK =  tick
handleCommand FULLTICK = fullTick
